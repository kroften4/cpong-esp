...
static bool send_state(int delta_time, void *room_p)
{
	...
	struct game_obj player1_upd =
		linear_move(state.player1, abs(input1.input_acc_ms));
	state.player2.velocity.y = state.player2.speed;
	state.player2.velocity.y *= get_input_acc_direction(input2.input_acc_ms);
	struct game_obj player2_upd =
		linear_move(state.player2, abs(input2.input_acc_ms));
	input1.input_acc_ms = 0;
	input2.input_acc_ms = 0;
	pthread_mutex_unlock(&input_mtx);

	struct wall wall = {
		.up = state.box_size.y, .down = 0, .left = 0, .right = state.box_size.x
	};
	struct game_obj ball_upd = { 0 };
	int scored_index;
	ball_advance(wall, state.player1, state.player2, state.ball, &ball_upd,
	...
	if (scored_index != -1) {
		...
		LOGF("sync change: %d -> %d", ssync, ssync + 1);
		ssync++;
	...
	pthread_mutex_lock(&sync_mtx);
	struct packet packet = { .type = packet_type,
							 .sync = ssync,
							 .data.state = state };
...
static bool on_input_ready(client_t client)
{
	...
	pthread_mutex_lock(&sync_mtx);
	if (packet.sync != ssync) {
		LOGF("invalid sync: got %d, current %d", packet.sync, ssync);
		pthread_mutex_unlock(&sync_mtx);
		return true;
	}
	pthread_mutex_unlock(&sync_mtx);
...
static void start_pong_game(struct room *room)
{
	...
	struct packet init_packet = { .type = PACKET_INIT,
								  .sync = ssync,
								  .data.state = state };
	for (int i = 0; i < ROOM_SIZE; i++) {
	...
	struct run_every_args state_sender_re_args = {
		.func = send_state, .args = room, .interval_ms = MIN_TICK_DURATION_MS
	};
