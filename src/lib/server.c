...
void server_handle_disconnection(server_t *server, client_t client)
{
	...
	if (client_node == NULL) {
		...
		__ts_queue_remove_nolock(server->clients, client_node->prev,
								 client_node->next);
	}
...
void *server_worker(server_t *server, void on_connection(client_t))
{
	for (;;) {
		...
		int connfd = accept(server->fd, (struct sockaddr *)&client_addr,
							&client_addrlen);
		if (connfd == -1) {
...
int connect_to_server(char *name, char *port)
{
	...
	serverfd = socket(server_ai.ai_family, server_ai.ai_socktype,
